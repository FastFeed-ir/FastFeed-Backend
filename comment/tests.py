...
class CommentTestCase(TestCase):
    ...
    def test_get_comment(self):
        ...
        self.assertEqual(response.data['order'], self.comment_data['order'])
        self.assertEqual(response.data['store'], self.comment_data['store'])
...
class RatingTestCase(TestCase):
    ...
    def test_get_rating(self):
        ...
        self.assertEqual(response.data['product'], self.rating_data['product'])
...
class RatingSerializerTestCase(TestCase):
    ...
    def test_serializer_with_valid_data(self):
        self.assertTrue(self.serializer.is_valid())
        rating = self.serializer.save()
        self.assertEqual(rating.score, self.rating_data['score'])
        self.assertEqual(rating.product_id, self.rating_data['product'])

    def test_serializer_with_missing_required_fields(self):
        invalid_data = {'score': 4}
        serializer = RatingSerializer(data=invalid_data)
        self.assertFalse(serializer.is_valid())
        self.assertEqual(serializer.errors['product'][0], 'This field is required.')
